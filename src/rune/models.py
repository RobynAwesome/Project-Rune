...
class Verifier:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.independence_basis:
            data["independence_basis"] = self.independence_basis
        return data

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> Verifier:
        # Migration: accept legacy independent_of_subject as claim alias.
        if "independence_claimed" in data:
            claimed = bool(data["independence_claimed"])
        elif "independent_of_subject" in data:
            claimed = bool(data["independent_of_subject"])
        else:
            claimed = False
        return cls(
            verifier_id=data["verifier_id"],
            verifier_type=data["verifier_type"],
            independence_claimed=claimed,
            independence_basis=data.get("independence_basis"),
        )
...
class EndorsementRecord:
    ...
    def create(
        ...
        *,
        ...
        signature: SignatureDict | str | None = None,
    ) -> EndorsementRecord:
        return cls(
    ...
    def from_dict(cls, data: dict[str, Any]) -> EndorsementRecord:
        ...
        verifier = Verifier.from_dict(data["verifier"])
        return cls(
